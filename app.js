...
async function exportPdf(optSnap=null){
  ...
  let fReg, fBold;
  try {
    const liberationBytes = await fetch('LiberationSans-Regular.ttf').then(r => { if (!r.ok) throw new Error(r.status); return r.arrayBuffer(); });
    fReg = await pdf.embedFont(liberationBytes, { subset:true });
    const bResp = await fetch('LiberationSans-Bold.ttf');
    if (bResp.ok) {
